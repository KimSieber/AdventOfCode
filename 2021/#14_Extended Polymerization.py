...
def processStep(poly, insertion):
    ...
    for key in poly:
        ...
        if new_keys[0] in poly_new.keys():      poly_new[new_keys[0]] += poly[key]
        else:                                   poly_new[new_keys[0]]  = poly[key]
        if new_keys[1] in poly_new.keys():      poly_new[new_keys[1]] += poly[key]
        else:                                   poly_new[new_keys[1]]  = poly[key]
    return poly_new
