...
interferences = [[int(point) for point in line.strip().split(',')] for line in open("#06 Input", "r")]


def buildBoards(interferences):
    sizeX  = max( [ x[0] for x in interferences ] ) + 2
    sizeY  = max( [ y[1] for y in interferences ] ) + 2 
    board1 = [ [ None for _ in range(sizeX) ] for _ in range(sizeY) ]
...
board1, board2 = buildBoards( interferences )
finite_points  = getFinitePoints( board1, interferences )
largest_area   = max( [ sum( [ sum( [ 1 for p in line if p==point ] ) for line in board1 ] ) for point in finite_points ] )
